/* ── STUDIO'S COPY ─────────────────────────────────────────────────────
 * Mirrored from pallefar/project-contract, flightdeck/web/src/motion/
 * motion.ts at commit 54064a53 (feat/anime-motion-os, "feat(motion): Atlas
 * overlay motion"). Studio is a separate repo, so it carries a copy of this
 * small module rather than depending across repos. Everything below is
 * byte-identical to that commit except, and only:
 *   1. `MOTION.duration.swap`, `MOTION.distance.swap` and `swapIn()`:
 *      Atlas's `studio-enter` (work-studio.css:74, 415-423), the panel swap
 *      Studio's tabs play. The OS working tree carries the same three,
 *      word for word, uncommitted at the time of this copy.
 *   2. Type-only edits for Studio's stricter tsconfig
 *      (`exactOptionalPropertyTypes`, and no `vite/client` types for
 *      `import.meta.env`). Each is marked "Studio:".
 * Re-sync by diffing against the OS file; keep the two in step.
 * ─────────────────────────────────────────────────────────────────── */
...
// Studio: no `vite/client` types here, and `?.` because a module run outside Vite has no `env`.
const ENV_FLAG = (import.meta as ImportMeta & { env?: Record<string, string | undefined> }).env?.["VITE_FD_MOTION_OFF"];
...
interface Drive {
  ...
  origin?: string | undefined; // Studio: `| undefined` for exactOptionalPropertyTypes
  ...
  onDone?: (() => void) | undefined; // Studio: `| undefined` for exactOptionalPropertyTypes
}
...
/** Does the stylesheet already transition this element's rotation? Then CSS
 * plays the turn and a JS turn on top would double it. */
 ...
 * stylesheet transitions the rotation itself. Ends handed back to the
 * stylesheet. */
